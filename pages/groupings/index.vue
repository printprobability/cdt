...
const {
  data: groupingsByBookID,
  refresh: refreshGroupingsByBookID
} = await useAsyncData("groupingsByBookID", () => queryContent("groupings")
  .where({characters: {$containsAny: () => charactersByBookID.map((x) => x.id)}})
  .find());
const {
  data: groupingsByCharacterClass,
  refresh: refreshGroupingsByCharacterClass
} = await useAsyncData("groupingsByCharacterClass", () => queryContent("groupings")
...
const {
  ...
  refresh: refreshFilteredCharacters
} = await useAsyncData("filteredCharacters", () => queryContent("characters")
